...
public class tpo implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if(!p.hasPermission("NTIR.Admin")){
            p.sendMessage(NTIRTP.format + ChatColor.RED + "你沒有執行此指令的權限!");
            return false;
        }

        if(args.length != 1){
            p.sendMessage(NTIRTP.format + ChatColor.RED + "請輸入要傳送的玩家!");
            return false;
        }
        ...
        p.sendMessage(NTIRTP.format + ChatColor.YELLOW + "你強制傳送到" + ChatColor.AQUA + target.getDisplayName() + ChatColor.YELLOW + "的身邊");
